...
void loop() {
  ...
  while (playcount == 0) { // Tunggu sampai tombol ditekan (LOW)
    if (blink) {
      lcd.setCursor(4, 0);
      lcd.print("Car Game");
      lcd.setCursor(0, 1);
    ...
    blink = !blink;
    // delay(400); // Berkedip setiap 500ms
  ...
  if (!playing)  {
    ...
    if (blink) {
      ...
      lcd.setCursor(0, 0);
      lcd.print("             "); // Hapus teks saat berkedip
    }
    delay(320);
    blink = !blink;
  ...
  if (buttonPushed) {
    ...
    tone(buzzerPin, 554, 120);
    //delay(100);  // Durasi buzzer 100ms
    //digitalWrite(buzzerPin, LOW);
  }

  if (drawCAR(CARPos, terrainUpper, terrainLower,  distance >> 3)) {
    digitalWrite(buzzerPin, HIGH);
    tone(buzzerPin, 659, 120);
    delay(200);
    tone(buzzerPin, 622, 120);
    delay(200);
    tone(buzzerPin, 587, 120);
    delay(200);
